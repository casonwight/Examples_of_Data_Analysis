# Part 1 (scroll down to see part 2)

# DATA
# Data is obtained from the NASA Goddard Institute for Space Studies (GISS)
# Data was accessed on 2/12/2018
# The following URLs are where the data is located:

# 1.      https://data.giss.nasa.gov/gistemp/tabledata_v3/GLB.Ts+dSST.txt
# 2.      ftp://aftp.cmdl.noaa.gov/data/trace_gases/co2/flask/surface/co2_mlo_surface-flask_1_ccgg_month.txt
# 3.      ftp://aftp.cmdl.noaa.gov/data/trace_gases/ch4/flask/surface/ch4_mlo_surface-flask_1_ccgg_month.txt

nasatemp<-read.table(text=
...
maunaloa1<-read.table(text="where year month co2
MLO 1969  8   322.51
                      MLO 1969  9   321.36
                      MLO 1969 10   320.74
                      MLO 1969 11   321.98
...
maunaloa2<-read.table(text = "where year month methane
MLO 1983  5  1639.23
                      MLO 1983  6  1633.53
                      MLO 1983  7  1633.19
                      MLO 1983  8  1631.38
...
plot(temp~co2, data= climate, main = "Temperature by CO2")
cor(climate$temp, climate$methane)
plot(temp~methane, data= climate, main = "Temperature by Methane")

# We know, from the correlation coefficients that there is a strong positive correlation
# between the greenhouse gas levels and the Earth's temperature


# ANALYSIS
# Response variable: Earth Temperature (monthly)
# Explanatory Variables: Earth's co2 (ppm) and methane (ppb) levels

# The model for temperature and its assumptions for inference:
# temp = beta0 + beta1 * co2 + beta2 * methane + epsilon, epsilon~N(0, sigma^2)
# Assumptions:
#
# We need an isolated way to collect the data so that it is not corrupted
# We need higher and higher (literal altitude) measurements of atmospheric conditions


# Estimate coefficients and standard errors by fitting the following model:
# temp = beta0 + beta1 * co2 + beta2 * methane + epsilon, epsilon~N(0, sigma^2)
climate.out <- lm(temp~co2 + methane, data = climate)
summary(climate.out)


# Explanation of beta1hat
# The beta1hat is the co2 coefficient estimate (.74107) so this is the estimated
# partial slope of co2, holding methane constant. 
# Better explanation: for a one unit change in co2 (ppm), there is an estimated expected
# increase of 0.74107 (.01 degrees C) in monthly earth temperature holding methane constant.


# Explanation of beta2hat
# The beta2hat is the methane coefficient estimate (.09444) so this is the estimated
# partial slope of methane, holding co2 constant. 
# Better explanation: for a one unit change in methan (ppb), there is an estimated expected
# increase of 0.09444 (.01 degrees C) in monthly earth temperature holding co2 constant.
...
# Test H0: no greenhouse effect
# Fit model assuming H0 is true

climate.out.null <- lm(temp~+1, data = climate)
anova(climate.out.null, climate.out)

# Hypotheses-
# H0: There is no greenhouse effect, so beta1 = beta2 = 0
# H1: There is a statistically significant greenhouse effect, so beta1 != 0 or beta2 != 0

# F-stat = 338.47
# P-value = 2.2e-16

# Conclusion-
# We reject the null hypothesis and say that there is a significant greenhouse effect from
# at least one of the two gasses on the temperature (p-value < .0001).
# To a non-statistician:
# There is a statistically significant greenhouse effect  on temperature (p-value < .0001)



# As follow up to concluding there is a statistically significant greenhouse gas effect:

# 95% confidence intervals
confint(climate.out)

# We want to know if there is a statistically significant greenhouse effect. We also want to know
# which of methane and co2 (or if both) have a significant effect on the temperature of the Earth.
# From the slope confidence intervals, we can see that methane has an interval (.0230 to .1589) that 
# is much closer to zero than the interval of co2 (.5463 to .9359). This means that co2 has a 
# greater effect on temperature than methane does. Neither methane nor co2 has an interval that 
# crosses over zero which means that both methane and co2 have significant effects on temperature, 
# even after adjusting each one for the other (p-value = 2.2e-16).

# For a one ppm change in co2, there is an estimated .7411 (95% CI: .5463 to .9359) degree change in global
# temperature. For a one ppb change in methane, there is an estimated .0944 (95% CI: .0230 to .1589)
# degree change in global temperature.
...
#     Predict temp for test using model estimated from train. Report summary statistics of 
#                 the 'absolute prediction error' distribution. Report results in oC.

out.train.climate <- lm(temp ~ co2 + methane, data = climate.train)

climate.prediction <- predict(out.train.climate, newdata = climate.test)
actualAndPreds <- cbind(climate.prediction, climate.test$temp)
plot(climate.prediction, climate.test$temp)
abline(0,1,col = "grey", lty = "dotted")

absError <- abs(actualAndPreds[,1]-actualAndPreds[,2])/100
summary(absError)

# Research Task and Data Features that Match Analysis Strengths:

# The task was to determine if there is significant statistical evidence of a greenhouse effect
# and which gas(ses) have a significant effect on temperature. 

# The regression model fits this task well, because we can easily analyze the two different effects
# and with more analysis, determine which have a significant effect. 

# Analysis Weaknesses:

# This analysis shows strong correlation, but may not necessarily show causal. Another weakness
# is that we are not including other greenhouse gasses or other possible factors in our analysis.


# Challenge:
# We could use the data set "Computer Hardware" from the UCI machine learning repository at 
# http://archive.ics.uci.edu/ml/datasets/Computer+Hardware, to see if channels and memory 
# have a significant effect on machine cycle time of computers.
...
# Cason Wight
# Data is obtained from the NASA Goddard Institute for Space Studies (GISS)
# Data accessed on 2/26/2018 from the following:
...
maunaloa3 <- read.table(header=TRUE,text="year month n2o sd n
...
maunaloa4<-read.table(header=TRUE,text="year month hcfc sd n
...
maunaloa5<-read.table(header=TRUE,text="year month sf6 sd n
1998       1     nan        nan       0
1998       2     nan        nan       0
1998       3     nan        nan       0
1998       4     nan        nan       0
...
# fit the model:
out.climate.moreVariables <- lm(temp~co2+methane+n2o+hcfc+sf6, data = climate)
summary(out.climate.moreVariables)

# Odd things:
#       co2 has the highest beta estimate, but is not significant
#       hcfc and sf6 have negative slope estimates (when correlation was positive) 
#       hcfc is also statistically significant



# We will check collinearity to compare the observational study to a designed experiment
plot(~co2+methane+n2o+hcfc+sf6, data = climate)
# Collinearity is a near linear relationship between explanatory variables
#   "lurking variables"

# collinearity diagnostic
library(car)
vif(out.climate.moreVariables)
# The only one that isn't bad is Methane

# There is a collinearity problem, so we are removing columns one-by-one and
# keeping the ones with least collinearity.

out.climate2 <- lm(temp~co2+methane+hcfc+sf6, data = climate)
summary(out.climate2)

vif(out.climate2)

out.climate3 <- lm(temp~co2+methane+hcfc, data = climate)
summary(out.climate3)

plot(~co2+methane+hcfc, data = climate)
vif(out.climate3)

out.climate4 <- lm(temp~methane+hcfc, data = climate)
summary(out.climate4)

plot(~methane+hcfc, data = climate)

# We will check to see if a transformation is in order
plot(temp~methane+hcfc, data = climate)
plot(temp~log(methane)+log(hcfc), data = climate)
plot(log(temp)~log(methane)+log(hcfc), data = climate)
# The log transformations look a lot more linear, so we will keep it as the model
out.climate5 <- lm(log(temp)~log(methane)+log(hcfc), data = climate)

# plot the points to check for outliers
par(mfrow = c(1,2))
plot(log(temp)~log(methane), data = climate)
identify(x= log(climate$methane), y = log(climate$temp))
# Spot potential outliers and influential points
pot.out.meth <- c(5, 54, 9, 7, 8, 87, 31, 32, 76, 182, 183, 178)

plot(log(temp)~log(hcfc), data = climate)
identify(x= log(climate$hcfc), y = log(climate$temp))
# Spot potential outliers and influential points
pot.out.hcfc <- c(32, 76, 183, 182, 5, 7, 9, 8, 54, 87, 93, 130)

potentials <- unique(c(pot.out.meth, pot.out.hcfc))

# Check leverage of the points
cutoff.lev <- 2*(2+1)/nrow(climate)
leverage.climate5 <- lm.influence(out.climate5)$hat
leverage.climate5[potentials]<cutoff.lev
# 183 needs to be thrown out.
toCutLev <- 183

# check cook's distance of the points
cd.climate5 <- cooks.distance(out.climate5)
cutoff.cd <- 4/(nrow(climate)-(2+1))
cd.climate5[potentials]<cutoff.lev
# 183, 182, 7, 9, 54, and 87 need to be thrown out.
toCutCD <- c(183, 182, 7, 9, 54, 87)

R.climate5 <- rstudent(out.climate4)
abs(R.climate5[potentials]) # chucks the most extreme potentially 1%
# 183, 182, and 87 need to be thrown out
toCutR <- c(183, 182, 87)

obsToCut <- unique(c(toCutLev, toCutCD, toCutR))
# in total, 183, 7, 9, 54, and 87 need to be thrown out

out.climate6 <- lm(log(temp)~log(methane)+log(hcfc), data = climate[-obsToCut,])
summary(out.climate6)

# Check collinearity of the new model. 
vif(out.climate6)

# In all, this new fit model (out.climate6) cuts all but two explanatory of 
# variablesthe data, which are the two with the least collinearity problems.
# It also excludes badly influential outliers based on cook's distance, the 
# R-studentized residuals, and the leverage. Notice that the beta's here are
# estimates of the transform slopes, so they represent percentage changes.
# There was negative slope in at least one beta until we reduced it to only
# methane and hcfc as the explanatory variables. 

# Research Task and Data Features that Match Analysis Strengths:
# Task- Fit a model that shows the effects of greenhouse gasses on the global
# temperature.

# Analysis strengths- 
# There is a lot of data to go on and with regreeion it is easy to see the  
# effects of any specific gas. We can also easily look at issues with collinearity
# and this model is rather intuitive.

# Analysis weaknesses- 
# There is such a large collinearity issue that we had to throw out most of 
# the explanatory variables. People are not as familiar with methane and hcfc
# as they are with, say, co2. 

# Challenge- 
# Fit a model that estimates life satisfaction, from a number of explanatory variables
# The data is found at http://psychstat3.missouristate.edu/Documents/MultiBook3/Mlt06.htm
